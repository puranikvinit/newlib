...
__inhibit_loop_to_libcall memmove (void *dst_void, const void *src_void, size_t length) {
	...
	if (src < dst && dst < src + length) {
		...
		/* Case: Destructive overlap, have to copy backwards. */
		src += length;
		dst += length;

		if (length >= SZREG) {

			if (__libc_aligned_xlen(dst, src)) {
			...
			else if (length > (SZREG*2)) { /* Case: At least one of the addresses is unaligned. */

				/* Align src so we can load one (double)word at a time. */
				...
				/* Copy SZREG bytes at a time if possible.
				   Load one (double)word at a time and store back individual bytes.
				 */
				do {	uintxlen_t src_xlen = *--aligned_src;
#if __riscv_xlen == 64
	...
	else { /* Case: Non-destructive overlap or no overlap at all. */
